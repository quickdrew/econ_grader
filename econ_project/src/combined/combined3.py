...
def calculate_total_change(data, start, end):
    ...
    normalized_total_change = (total_change / max(quarters_in_office, 1)) * 30
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            dcc.Checklist(
                id='ignore-covid-checklist',
                options=[
                    {'label': 'Ignore COVID-19 Period', 'value': 'ignore_covid'}
                ],
                value=[],
                style={'color': 'grey', 'margin-top': '20px'}
            ),
            html.Label('COVID Start Date:', style={'color': 'grey', 'margin-top': '10px'}),
            dcc.Input(
                id='covid-start-date',
                type='text',
            ...
            html.Label('COVID End Date:', style={'color': 'grey'}),
            dcc.Input(
                id='covid-end-date',
                type='text',
                placeholder='YYYY-MM-DD',
        ...
        ], style={'display': 'inline-block', 'backgroundColor': '#333', 'color': 'grey', 'padding': '10px', 'border': '1px solid #444', 'width': '30%', 'margin-left': '10px', 'vertical-align': 'top'})
    ], style={'width': '100%', 'padding': '20px 0'}),
    dcc.Graph(id='percentile-plot', style={'height': '75vh'}),
    html.Label('AUC vs Total Change Weight:', style={'color': 'grey', 'margin-top': '20px'}),
    dcc.Slider(
...
@app.callback(
    ...
    [Input('gdp-weight-slider', 'value'),
     ...
     Input('ignore-covid-checklist', 'value'),
     Input('covid-start-date', 'value'),
     Input('covid-end-date', 'value')]
)
def update_plot(gdp_weight, employment_weight, inflation_weight, auc_total_change_weight, hover_data, ignore_covid_checklist, covid_start_date, covid_end_date):
    ...
    # Ignore COVID-19 period if checkbox is checked
    filtered_data = combined_data.copy()
    if 'ignore_covid' in ignore_covid_checklist and covid_start_date and covid_end_date:
        covid_start = pd.to_datetime(covid_start_date)
        covid_end = pd.to_datetime(covid_end_date)
        filtered_data = filtered_data[~((filtered_data.index >= covid_start) & (filtered_data.index <= covid_end))]
    ...
    fig.add_trace(go.Scatter(
        ...
        y=filtered_data['Weighted_Average_Percentile'],
        mode='lines',
        name='Weighted Average Percentile',
        text=filtered_data.apply(lambda row: f"Weighted Average Percentile: {row['Weighted_Average_Percentile']:.2f}", axis=1),
        hoverinfo='text'
    ))
    ...
    if hover_data is not None and 'points' in hover_data and len(hover_data['points']) > 0:
        ...
        hover_date = filtered_data.index[hover_point_index]
        hover_data_info = filtered_data.iloc[hover_point_index]
    else:
        # Default to the latest data if no hover
        hover_data_info = filtered_data.iloc[-1]
        hover_date = filtered_data.index[-1]
    ...
    for president in presidents:
        ...
        auc_value = calculate_auc(filtered_data, start, end)
        total_change = calculate_total_change(filtered_data, start, end)
