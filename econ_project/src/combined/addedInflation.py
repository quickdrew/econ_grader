...
# Filter data after 1960
inflation_data = inflation_data[inflation_data.index >= '1960-01-01']
...
inflation_data['Inflation_Percentile'] = inflation_data['Inflation'].rank(pct=True)

# Merge GDP, unemployment, and inflation data for averaging
combined_data = pd.merge(gdp_data[['YoY_GDP_Growth', 'Growth_Percentile']], unemployment_data[['UNRATE', 'Employment_Percentile']], left_index=True, right_index=True, how='outer')
combined_data = pd.merge(combined_data, inflation_data[['Inflation', 'Inflation_Percentile']], left_index=True, right_index=True, how='outer')
